...
from model.TimeXer_mod_code import Model
...
X_train, X_test, y_train, y_test, mod_train, mod_test = train_test_split(
    X_tensor, y_tensor, mod_type_tensor, test_size=0.2, random_state=42
)

# 构建 TensorDataset（包含 3 个输入）
train_dataset = TensorDataset(X_train, y_train, mod_train)
test_dataset  = TensorDataset(X_test, y_test, mod_test)
...
for epoch in range(300):
    ...
    for inputs, targets, mode_type in tqdm(train_loader):
        inputs = inputs.cuda(non_blocking=True)
        targets = targets.float().cuda(non_blocking=True)  # 确保标签为浮点
        mod_types = mode_type.cuda()
        # 混合精度训练
        with torch.cuda.amp.autocast():
            outputs = model(inputs, mod_types)
    ...
    count=0
    with torch.no_grad():
        for inputs, targets, mode_type in tqdm(val_loader):
            inputs = inputs.cuda()
            targets = targets.float().cuda()
            mod_types = mode_type.cuda()
            
            outputs = model(inputs, mod_types,inference=True)
            cut_targets,cut_outputs = truncate_batch_pairs(targets,outputs)
            val_loss_list = []
            cq_score_list = []
            for output_sample, target_sample in zip(cut_outputs, cut_targets):
    ...
    # 定义保存模型的文件夹路径
    save_folder = "SOTA_with_normlization_with_multi_class"  # 可根据需要修改文件夹名称
